...
export default function UpdateForm({ config, formElements }) {
  ...
  const onSubmit = (fieldsValue) => {
    if (fieldsValue) {
      ...
      if (fieldsValue.receivedDate) {
        fieldsValue = {
          ...fieldsValue,
          receivedDate: fieldsValue["receivedDate"].format("DD/MM/YYYY"),
        };
      }
      if (fieldsValue.expiryDate) {
        fieldsValue = {
          ...fieldsValue,
          expiryDate: fieldsValue["expiryDate"].format("DD/MM/YYYY"),
        };
      }
    }
  ...
  useEffect(() => {
    if (current) {
      ...
      if (current.receivedDate) {
        current.receivedDate = dayjs(current.receivedDate, "DD/MM/YYYY");
      }
      if (current.expiryDate) {
        current.expiryDate = dayjs(current.expiryDate, "DD/MM/YYYY");
      }
